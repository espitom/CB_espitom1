...
//     let(let i =0; i<array2_subarrays.length; i += 2)
//     {
...
// fubction quickSortPartition(items, left, right){
...
//     return items: // this returns the sorted arrays
// }

// let items1 = [5,3,7,6,2,9];
// console.log(quickSort(items1, 0, items.length -1));
...
/*
    ...
    str = string
    match() take in
    regexp - regular expression object(Argument is implicitly converte to regular expression if it is a non regular expression)
...
//without using g flag - we only get the first match but with detailed information like index, input and groups
console.log(result);

const re1 = /Java/g;
let result1 = string.match(re1)

console.log("Result of matching /Java/ with a gflag:");
//with using gflag it will return only the  first match.
console.log(result1);

//groups - an object of named capturing groups having keys as the nmaes and values as the captured matches.
//index - index of the search where a result was found
//input - the copy of the search string

//Matching sections in a string
const sentence = "My name is Jelly. Your name is Ivan";

//expression matches case-sensitive name + any alphabets still period
const reg = /name\sis\[a-zA-z]+\./gi;

let res = sentence.match(reg);
console.log(res);//['name is Jelly., 'name is Ivan.']

//using named capturing groups
const reg1 = /name\sis\s(?<name>[a-zA-z]+)\./i;
let found = sentence.match(reg1);console.log(found.groups)//object: {name: 'Jelly'}
